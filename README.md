...
> напоминания «заберите технику», **183 pytest-тестов**.
...
## Приёмка мастером: доступ к своей приёмке

Исполнителя на ремонт назначает администратор или оператор (`ASSIGN_ROLES`),
поэтому у ремонта, который мастер принял сам, `master_id` пустой. Чтобы мастер
не терял собственную приёмку, доступ и печать определяются ещё и по факту
приёмки (`accepted_by`):

- `can_access_repair()` и `can_print()` — мастер видит и печатает свой ремонт
  **или свою приёмку**; чужие неназначенные ремонты ему по-прежнему закрыты
  (403/404, очередь печати — 403, потому что в ней PDF с чужими ПДн);
- `_master_scope()` — то же условие в списках, на доске и в счётчиках этапов,
  иначе только что созданный ремонт мгновенно пропадал у мастера из вида;
- автопечать этикетки при приёмке поэтому работает у всех ролей, включая
  мастера: `POST /api/repairs/{id}/print-label` больше не упирается в 403.

Поиск в «Все ремонты» (`?q=`) теперь учитывает номер ремонта и серийник — их
читают с этикетки на технике (раньше в условии не было `Repair.number`, и поиск
по номеру не работал).
...
- [x] **Тесты**: 183 pytest-тестов (auth/чат/ремонты/прайс/склад/оплаты/админ/
