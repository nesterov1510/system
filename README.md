...
Мастер видит **только свои ремонты и свободные** (без исполнителя) — свободные
затем, чтобы взять заказ себе. Чужие ремонты с назначенным исполнителем ему не
показываются: ни в таблице, ни на доске, ни в счётчиках этапов, и карточка
такого ремонта отвечает 403. Старшие роли (администратор, оператор, менеджер)
видят всё.

Границы заданы один раз в `app/services/repair_scope.py`
(`master_visible(user_id)` = назначенные мастеру **или** свободные), поэтому
