...
> напоминания «заберите технику», **184 pytest-тестов**.
...
## Приёмка мастером: печать есть, чужих заказов не видно

Исполнителя на ремонт назначает администратор или оператор (`ASSIGN_ROLES`),
поэтому у ремонта, который мастер принял сам, `master_id` пустой. Правила:

- **мастер видит только назначенные ему заказы** — `can_access_repair()` и
  `_master_scope()` (список «Все ремонты», доска, счётчики этапов) работают
  строго по назначению (`master_id` / `repair_masters`). Своя приёмка без
  исполнителя у него в списке не висит;
- **этикетка своей приёмки печатается** — `can_print()` разрешает приёмщику
  ...
  заказ передали другому мастеру, печать и доступ у приёмщика закрываются;
- чужие неназначенные ремонты мастеру закрыты (403/404), очередь печати
  `/print/jobs` — тоже 403 (там PDF с чужими персональными данными).
...
- [x] **Тесты**: 184 pytest-тестов (auth/чат/ремонты/прайс/склад/оплаты/админ/
