...
Админ может переставить блоки страницы под себя: на **карточке ремонта** и во
вкладке **«Все ремонты»** над контентом есть кнопка **«🧩 Конструктор
блоков»**. Включается режим правки: у каждого блока появляется строка с
названием и стрелками ↑ ↓, а сами блоки можно таскать мышью. Кнопка
**«💾 Сохранить»** записывает порядок, **«↺ По умолчанию»** возвращает
заводской.
...
`legend`, `bulk` (массовые действия), `table`, `pager`.
...
`.rcard` (двухколоночный grid, `.is-wide` — во всю ширину). Логика —
`app/webui/layout.py`: `normalize()` отбрасывает чужие имена и дописывает в
конец новые блоки, поэтому после обновления страницы сохранённая раскладка не
ломается и не «съедает» свежие блоки.
