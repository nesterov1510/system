...
Админ может переставить под себя и блоки страницы, и колонки таблицы: на
**карточке ремонта** и во вкладке **«Все ремонты»** над контентом есть кнопка
**«🧩 Блоки страницы»**, а в списке дополнительно **«🔀 Колонки таблицы»**.
Включается режим правки: у каждого блока появляется строка с названием и
стрелками ↑ ↓, у каждого заголовка колонки — ручки ← →; и то и другое можно
тянуть мышью. Кнопка **«💾 Сохранить»** записывает порядок, **«↺ По
умолчанию»** возвращает заводской.
...
встали в сохранённый порядок (без node/jsdom тест честно пропускается).
