...
- **назначать исполнителей** — `can_assign_repair_masters(user, repair)`:
  администратор и оператор всегда; мастер — на свободный ремонт (берёт себе,
  кнопка «🙋 Взять себе» прямо в строке списка) или на свой (добавляет
  напарников и помощников). Занятый чужой заказ мастер себе не переписывает;
