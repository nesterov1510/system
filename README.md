...
Мастер видит **весь реестр** — иначе он не найдёт свободные заказы. Права
разделены на «смотреть» и «менять»:

- **смотреть** — `can_view_repair()`: карточку любого ремонта открыть можно;
