...
список исполнителей `repair_masters`, в том числе как помощник) **либо** он сам
принял технику (`accepted_by`). **Свободные** — исполнитель не назначен, принял
не мастер (приёмка оператора/админа или публичной приёмки, ушедшая в общую
очередь), и ремонт ещё не завершён и не выдан.
