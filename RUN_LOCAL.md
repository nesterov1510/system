...
### Способ 0 (сервер MSB): бланки в CUPS, этикетки — raw TSPL

Бланки A4 печатаются в локальную очередь CUPS `office_printer_a4`: в админке
**Режим** = «CUPS на этом сервере», **Имя очереди** = имя из `lpstat -p`,
агент выполнит `lp -d office_printer_a4 <бланк.pdf>`.

Этикетки 58×38 уходят **напрямую по сети** в принтер TSPL
`192.168.8.75:9100` (он не в CUPS): в админке **Режим** = «Напрямую по сети
(TSPL, порт 9100)», **Адрес** = `192.168.8.75`, **Порт** = `9100`. MSB
собирает этикетку в монохромный растр и шлёт её командой `BITMAP`.
`MSB_PRINT_CMD` в обоих режимах не участвует. Подробности — в
