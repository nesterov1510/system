...
Всё работает без Docker: backend + веб-интерфейс (FastAPI + Jinja2/HTMX + SQLite),
...
`{"status":"ok",...}`. Веб-интерфейс: http://localhost:8085/login.
Swagger (документация API): http://localhost:8085/docs
...
## 3. Веб-интерфейс (тот же порт 8085)

Отдельный фронтенд не нужен: Jinja2/HTMX UI отдаёт сам API. После запуска
backend откройте http://localhost:8085/login — экран входа MSB.

JSON-API остаётся на `/api/*`, медиа — на `/media/*`, чат — на `/ws`.
...
Есть **три способа** подключить принтер — выбирается в админке
(`Админ → Настройки → Принтер`) для каждой очереди отдельно.

### Способ 0 (сервер MSB): очередь CUPS на этом же сервере

Если принтер добавлен в CUPS той же машины, где работает MSB, достаточно имени
очереди — адрес знает CUPS. Так настроены оба принтера сервера:

| Что печатаем | Очередь CUPS |
|---|---|
| Бланки приёмки A4 | `office_printer_a4` |
| Этикетки 58×38 мм | `3B-350B` |

В админке: **Режим печати** = «На этом сервере (достаточно имени очереди)»,
**Имя очереди** = имя из `lpstat -p`, IP и порт не заполняются. Агент выполнит
`lp -d office_printer_a4 <бланк.pdf>` и
`lp -d 3B-350B -o media=Custom.58x38mm <этикетка.pdf>`; `MSB_PRINT_CMD` в этом
режиме не участвует. Подробности — в [`LABEL_PRINTER.md`](LABEL_PRINTER.md).

### Способ A: через драйвер ОС + print-agent
...
4. **Режим печати** в админке: на сервере MSB — «На этом сервере» с очередью
   `office_printer_a4`; на Windows надёжнее `agent` + SumatraPDF.
...
1. Откройте http://localhost:8085/login → войдите как `admin@msb.local` / `admin123`.
...
4. «Приёмка» → заполните клиента и технику → «Принять технику». Дальше всё само:
   этикетка 58×38 уходит на принтер, появляется окно «Данные успешно сохранены /
   этикетка напечатана — проверьте принтер», и через 6 секунд система переходит
   на доску «Все ремонты». Если принтер этикеток не настроен, окно покажет
   ошибку и останется на месте — можно повторить печать или уйти на доску вручную.
...
> живёт на API, порт 8085). Без этого QR с телефона клиента не откроется.

### Шаг 3. Запустите сервер

Сервер уже слушает все сетевые интерфейсы (`0.0.0.0`):
...
- Телефон обращается к UI/API по `http://192.168.8.81:8085`.
- Страницы, `/api`, `/media` и `/ws` — same-origin, CORS не нужен.
- Чат (WebSocket) использует адрес, по которому открыта страница.
...
Если с телефона не открывается — разрешите входящие соединения на порт
**8085** в брандмауэре/файрволе ОС (Windows Firewall, ufw и т.п.).
...
→ проверьте, что print-agent запущен, а имя очереди в «Админ → Настройки →
Принтер» совпадает с выводом `lpstat -p` (в режиме «Драйвером ОС» — что
`MSB_PRINT_CMD` указывает на существующий принтер). Логи agent выводит в
консоль.
...
|---|---|---|
| Веб-интерфейс | http://localhost:8085/login | http://IP-машины:8085/login |
