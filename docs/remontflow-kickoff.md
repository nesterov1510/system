# RemontFlow — Kickoff / ТЗ-ответ на суперпромпт
...
| # | Название | Slug | Смысл |
|---|---|---|---|
| 1 | **RemontFlow** | `remontflow` | «Поток ремонтов» — рабочее имя, отражает конвейер «приёмка → ремонт → выдача» |
...
Рекомендация: **RemontFlow** (уже укоренилось в ТЗ). Домен: `remontflow.ru` (или `remont.{город}.ru`). Slug используется в URL монолита и в номерах ремонта (`TV-MSK-…` → заменить город на slug точки).
...
└── docs/
    └── remontflow-kickoff.md    # этот документ
```
