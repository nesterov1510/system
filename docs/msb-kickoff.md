...
**Главный поток.** Приёмка (мобильная форма → номер `TV-{city}-{YYYY}-{NNNNN}` → запись в БД → timeline → job печати → QR-бланк) → жизненный цикл статусов (Новый → На диагностике → В работе / Ждёт запчастей → Завершён; выдачу техники отмечает `issued_at`) → call-центр (очереди «согласовать/готово/просрочка») → прайс API → статистика+AI.
