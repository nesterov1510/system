...
## 8.1. Клиентская часть на втором сервере (без базы)

Схема: внутренний сервер (`192.168.8.81`) держит PostgreSQL, приёмку, админку и
API — и остаётся в локальной сети. Вторая машина отдаёт в интернет ровно одну
вещь — страницу статуса ремонта `/r/{token}`, которую клиент открывает по QR.
Базы на ней нет: статус она берёт у внутреннего сервера по HTTP.

Клиентской странице нужно всего три пути: `/r/{token}`, `/api/public/r/{token}`
и `/static/msb/base.css` (плюс `/health` для проверок). Страница рендерится на
сервере, без JS и без фото.

**Что закрывает `PUBLIC_ONLY=true`**

| Путь | Ответ |
|---|---|
| `/r/{token}` | 200 |
| `/api/public/r/{token}` | 200 |
| `/static/msb/base.css`, `/static/app.css` | 200 |
| `/health` | 200 |
| `/`, `/login`, `/repairs`, `/admin/*`, `/api/*`, `/docs`, `/openapi.json`, прочее `/static/msb/*`, `/media/*` | 404 |

`404`, а не `403` — снаружи не должно быть видно, что за ним есть другие
разделы. В этом режиме приложение не создаёт схему, не запускает seed и
рассылку напоминаний и не читает правила IP-контроля, то есть к базе не
обращается вовсе.

**Что включает доступ к внутреннему серверу**

`PUBLIC_UPSTREAM_URL=http://192.168.8.81:8085` — с ним `/r/{token}` и
`/api/public/r/{token}` проксируются с внутреннего сервера. Если внутренний
сервер недоступен, клиент видит «Сервис временно недоступен» и `502`, а не
«Ремонт не найден»: ссылка при этом может быть рабочей.

### Установка на второй машине

```bash
# код тот же, что и на внутреннем сервере
sudo cp deploy/msb-public.service /etc/systemd/system/
sudo cp deploy/env.public /home/windowrepair-ae/msb/.env.public
sudo chmod 600 /home/windowrepair-ae/msb/.env.public
# заполнить: PUBLIC_UPSTREAM_URL, CLIENT_BASE_URL, SECRET_KEY
sudo systemctl daemon-reload && sudo systemctl enable --now msb-public
```

Экземпляр слушает `127.0.0.1:8086`; наружу его выпускает край.

### Край: доступ по IP без домена

Без домена сертификат не выпустить, поэтому клиент открывает `http://IP:порт`
(браузер покажет «не защищено» — для страницы статуса это терпимо, паролей и
платежей там нет). Два варианта:

```bash
# А. nginx на второй машине, порт 80 открыт в интернет
...
# в server_name вписать IP второй машины (или _), CLIENT_BASE_URL=http://IP

# Б. Cloudflare Tunnel — порты не открываются, работает даже за NAT
...
Оба конфига пропускают только `/r/`, `/api/public/`, CSS страницы и `/health`;
на всё остальное отвечают `404`. Файрвол второй машины: наружу только `80`/`443`
(вариант А), порт `8086` не публикуется. Первая машина снаружи не видна вовсе.

### QR должен вести на вторую машину

Ссылку клиента строит `public_status_url()`. По умолчанию она берёт адрес из
запроса печати, то есть адрес оператора во внутренней сети, — по такому QR
клиент с мобильного интернета ничего не откроет. В `.env` **внутреннего**
сервера:

```
CLIENT_BASE_URL=http://IP_ВТОРОЙ_МАШИНЫ
```

На внутренние ссылки (`/repairs/{id}` в панели) переменная не влияет. После
изменения QR меняется только на новых бланках — уже распечатанные ведут туда,
куда были напечатаны.

### Проверка

```bash
# со второй машины: внутренний сервер отвечает
curl -s -o /dev/null -w "%{http_code}\n" http://192.168.8.81:8085/api/public/r/<токен>   # 200
# локально на второй машине
curl -s -o /dev/null -w "%{http_code}\n" http://127.0.0.1:8086/r/<токен>                 # 200
curl -s -o /dev/null -w "%{http_code}\n" http://127.0.0.1:8086/login                     # 404
# с телефона через мобильный интернет (не из Wi-Fi сервиса)
curl -s -o /dev/null -w "%{http_code}\n" http://IP_ВТОРОЙ_МАШИНЫ/r/<токен>               # 200
curl -s -o /dev/null -w "%{http_code}\n" http://IP_ВТОРОЙ_МАШИНЫ/api/repairs             # 404
```

Отдельная проверка, что база публичному серверу не нужна: поднимите его с
заведомо нерабочим `DATABASE_URL` — страница клиента обязана открываться, пока
внутренний сервер на связи.
