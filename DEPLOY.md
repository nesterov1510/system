...
## ⚠️ Важно перед первым продакшен-запуском

В `/home/windowrepair-ae/msb/.env` обязательно замените `SECRET_KEY` и `SEED_ADMIN_PASSWORD` на уникальные значения. Для установки PWA на телефоне браузер должен видеть сайт по HTTPS (либо через localhost при разработке); обычный HTTP по LAN может открыть сайт, но не показать кнопку установки.
