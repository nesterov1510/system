...
DEVICE_CLASSES = [
    ...
    {"value": "Мониторы", "label": "Мониторы", "icon": "🖥️"},
    {"value": "ТВ-приставки", "label": "ТВ-приставки", "icon": "📡"},
    {"value": "Компьютеры", "label": "Компьютеры", "icon": "💻"},
...
# Состояние техники: готовые отметки для чипа «Состояние» в карточке ремонта.
# Отмеченное склеивается в `condition_notes` через запятую; рядом есть поле
# «своими словами» для всего, чего нет в списке.
CONDITION_OPTIONS = [
    "Без видимых повреждений", "Царапины", "Сколы", "Трещины",
    "Разбит экран / матрица", "Вмятины", "Следы вскрытия", "Следы влаги",
]

# Комплектация по умолчанию: что обычно приезжает вместе с техникой.
# Список дополняется справочником complectation_items из БД.
DEFAULT_COMPLECTATION = [
    "Пульт", "Кабель питания", "Подставка", "Ножки", "Крепёж",
    "Блок питания", "Аккумулятор", "Документы", "Коробка",
]
